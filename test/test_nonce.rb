...
module OpenID
  class NonceTestCase < Test::Unit::TestCase
    ...
    def test_split
      ...
      expected_t = 0
      expected_salt = ''
      actual_t, actual_salt = split_nonce(s)
    ...
    def test_mk_split
      t = 42
      nonce_str = mk_nonce(t)
      assert(nonce_str.match(NONCE_RE))
      at, salt = split_nonce(nonce_str)
    ...
    def test_bad_split
      ...
      cases.each{|c|
        assert_raises(ArgumentError, c.inspect) { split_nonce(c) }
      }
    end
    ...
    def test_check_timestamp
      ...
      cases.each{|c|
        ...
        actual = check_timestamp(nonce_str, allowed_skew, now)
