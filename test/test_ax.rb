...
module OpenID
  ...
  class AXMessageTest < Test::Unit::TestCase
    ...
    def test_check_mode
      assert_raises(AXError) { @bax.do_check_mode({'mode' => 'fetch_request'})}
  ...
  class ToTypeURIsTest < Test::Unit::TestCase
    ...
    def test_empty
      [nil, ''].each{|empty|
        uris = to_type_uris(@aliases, empty)
        assert_equal([], uris)
      }
    end
    ...
    def test_undefined
      assert_raises(IndexError) {
        to_type_uris(@aliases, 'http://janrain.com/')
      }
    end
    ...
    def test_one
      ...
      uris = to_type_uris(@aliases, name)
    ...
    def test_two
      ...
      uris = to_type_uris(@aliases, [name1, name2].join(','))
  ...
  class ParseAXValuesTest < Test::Unit::TestCase
    ...
    def test_invalid_alias
      ...
      types.each{|t|
        inputs.each{|input|
          msg = t.new
          assert_raises(AXError) {msg.parse_extension_args(input)}
        }
      }
    end
  ...
  class FetchRequestTest < Test::Unit::TestCase
    ...
    def test_parse_extension_args_extra_type
      ...
      assert_raises(AXError) {@msg.parse_extension_args(args)}
    end
  ...
  class FetchResponseTest < Test::Unit::TestCase
    ...
    def test_get_single_extra
      ...
      assert_raises(AXError) { @msg.get_single(@type_a) }
    end
