...
module OpenID
  ...
  AX_UNLIMITED_VALUES = "unlimited"
  AX_MINIMUM_SUPPORTED_ALIAS_LENGTH = 32

  # check alias for invalid characters, raise AXError if found
  def check_alias(name)
    raise AXError, "Alias '#{name}' must not contain a comma or period." if name.match(/(,|\.)/)
  end

  # Raised when data does not comply with AX 1.0 specification
  class AXError < ArgumentError
  end
  ...
  class AXMessage < Extension
    ...
    def check_mode(ax_args)
      ...
      if actual_mode != @mode
        raise AXError, "Expected mode #{mode.inspect}, got #{actual_mode.inspect}"
      end
    end
  ...
  def to_type_uris(namespace_map, alias_list_s)
    ...
    def self.from_openid_request(message)
      ...
      if req.update_url
        ...
        if realm.nil? or realm.empty?
          raise AXError, "Cannot validate update_url #{req.update_url.inspect} against absent realm"
        end
        tr = TrustRoot::TrustRoot.parse(realm)
        unless tr.validate_url(req.update_url)
          raise AXError, "Update URL #{req.update_url.inspect} failed validation against realm #{realm.inspect}"
        end
      end
    ...
    def parse_extension_args(ax_args)
      ...
      ax_args.each{|k,v|
        if k.index('type.') == 0
          ...
          count = 1
          if count_s
            if count_s == AX_UNLIMITED_VALUES
              count = count_s
            else
              count = count_s.to_i
              if count <= 0
                raise AXError, "Invalid value for count #{count_key.inspect}: #{count_s.inspect}"
              end
            end
          end
      ...
      required = to_type_uris(aliases, ax_args['required'])
      required.each{|type_uri|
        @requested_attributes[type_uri].required = true
      }
      if_available = to_type_uris(aliases, ax_args['if_available'])
      all_type_uris = required + if_available

      aliases.namespace_uris.each{|type_uri|
        unless all_type_uris.member? type_uri
          raise AXError, "Type URI #{type_uri.inspect} was in the request but not present in 'required' or 'if_available'"
        end
      }
  ...
  class AXKeyValueMessage < AXMessage
    ...
    def get_single(type_uri, default = nil)
      ...
      if values.size != 1
        raise AXError, "More than one value present for #{type_uri.inspect}"
      else
        return values[0]
      end
    end
    ...
    def get_extension_args(request = nil)
      ...
      if request
        ...
        request.attributes.each{|attr_info|
          ...
          if attr_info.count != AX_UNLIMITED_VALUES and attr_info.count < values.size
            raise AXError, "More than the number of requested values were specified for #{attr_info.type_uri.inspect}"
          end
        }
      end
  ...
  class StoreResponse < AXMessage
    ...
    def initialize(succeeded = true, error_message = nil)
      ...
      if succeeded and error_message
        raise AXError, "Error message included in a success response"
      end
      if succeeded
        @mode = SUCCESS_MODE
      else
