...
module OpenID
  ...
  SREG_NS_URI_1_0 = 'http://openid.net/sreg/1.0'
  ...
  class SRegNamespaceError < ArgumentError
  end
  ...
  class SRegResponse < Extension
    ...
    # Take a SRegRequest and a hash of simple registration
    # values and create a SRegResponse object containing that data.
    ...
    # Create an SRegResponse object from an
    ...
    def self.from_success_response(success_response, signed_only = true)
      ...
      args.reject!{|k,v| !SREG_DATA_FIELDS.member?(k) }
      new(args, ns_uri)
    end
